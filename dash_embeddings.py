...
def create_scatter(data_df=DATA_DF):
    fig = px.scatter(data_df, x="x", y="y", title="UMAP", opacity=0.3, color="IDR")
...
def update_profile(hoverData, data_df=DATA_DF, bin_size=256):
    ...
    print(coord)
    fig = px.line(
        x=np.arange(len(true_profile)),
        y=true_profile,
        title="{}; PR={}".format(coord, str(np.round(pr, 3))),
    )
    fig.add_scatter(
        x=np.arange(len(pred_profile)),
        y=pred_profile,
        name="Predicted",
        mode="lines",
