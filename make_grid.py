...
# inputs
inputs_all = [
    os.path.join(all_data, foldername)
    for foldername in os.listdir(all_data)
    if "w_" in foldername
]
inputs_basenji = [
    os.path.join(all_data, foldername)
    for foldername in os.listdir(all_data)
    if "w_32" in foldername
]
inputs_bpnet = [
    os.path.join(all_data, foldername)
    for foldername in os.listdir(all_data)
    if "w_1" in foldername and "w_128" not in foldername
]
...
for input in inputs_all:
    for loss in losses:
        if loss != "poisson":
            cmd.append(
                "./train.py {} bpnet {} -e {} -o {}".format(
                    input, loss, epoch_n, output_dir
                )
            )
...
for input in inputs_all:
    for loss in losses:
        if "w_1" in input:
            ...
            cmd.append(
                "./train.py {} basenjimod {} -e {} -o {} ".format(
                    input, loss, epoch_n, output_dir
                )
            )
