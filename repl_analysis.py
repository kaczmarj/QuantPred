...
def summarize_replicates(run_dir, N_cell_line, bin_size):
    label_dict = {
        "threshold_2": {"bw": "{}_thresh2.bw".format(bin_size), "bed": "thresh2.bed"},
    ...
    folders_with_cell_line_N = [
        f for f in os.listdir(bigwigs_dir) if f.split("_")[0] == str(N_cell_line)
    ]
    print(folders_with_cell_line_N)
    assert (
        len(folders_with_cell_line_N) == 1
    ), "Folders for one cell line not identified!"
    ...
    for title, fileid in label_dict.items():
        print(res_dir)
        bw_paths = [
            os.path.join(res_dir, f) for f in os.listdir(res_dir) if fileid["bw"] in f
        ]
        bed_path = [
            os.path.join(res_dir, f) for f in os.listdir(res_dir) if fileid["bed"] in f
        ][0]
        print("*********")
        print(len(bw_paths))
        print(bw_paths)
        ...
        for x_lab, y_lab in comb_of_columns:
            pr_result = stats.pearsonr(
                all_vals_dict_1d[x_lab], all_vals_dict_1d[y_lab]
            )[0]
