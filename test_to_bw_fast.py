...
def read_model(run_path, compile_model=True):
    ...
    trained_model = tf.keras.models.load_model(
        model_path, custom_objects={"GELU": GELU}
    )
    if compile_model:
...
def open_bw(bw_filename, chrom_size_path):
    ...
    assert not os.path.isfile(bw_filename), "Bw at {} alread exists!".format(
        bw_filename
    )
...
def remove_np_nans(all_vals_dict):
    assert (
        "pred" in all_vals_dict.keys() and "truth" in all_vals_dict.keys()
    ), "Wrong keys!"
    include_rows = np.ones((all_vals_dict["pred"].shape[0]), dtype=bool)
    both_arrays = np.concatenate(
        (all_vals_dict["pred"], all_vals_dict["truth"]), axis=1
    )
...
def make_truth_pred_bws(
    truth_bw_filename_suffix,
    pred_bw_filename_suffix,
    bed_filename_suffix,
    testset,
    trained_model,
    bin_size,
    ...
    for cell_line, cell_line_name in enumerate(cell_line_names):
        output_dir = util.make_dir(
            os.path.join(run_dir, str(cell_line) + "_" + cell_line_name)
        )
        pred_bw_filename = os.path.join(
            output_dir, cell_line_name + pred_bw_filename_suffix
        )
        pred_bws[cell_line] = open_bw(pred_bw_filename, chrom_size_path)
        truth_bw_filename = os.path.join(
            output_dir, cell_line_name + truth_bw_filename_suffix
        )
    ...
    for C, X, Y in testset:  # per batch
        ...
        for i, pred in enumerate(P):  # per batch element
            ...
            for cell_line in range(cell_line_N):  # per cell line
                ...
                truth_bws[cell_line].addEntries(
                    chrom,
                    start,
...
def merge_bed(in_bed_filename):
    ...
    in_bed_filename_merged = "/".join(
        split_filename[:-1] + ["merged_" + split_filename[-1]]
    )
    # str command line for bedtools merge
    bashCmd = "bedtools merge -i {} > {}".format(
        in_bed_filename, in_bed_filename_merged
    )
...
def scipy_get_pr(
    bw_paths,
    bedfile="/home/shush/profile/QuantPred/bin_exp/truth/merged_truth_1_raw.bed",
):
    ...
    assert (
        len(all_vals_dict_1d[bw_paths[0]]) > 1
        and len(all_vals_dict_1d[bw_paths[1]]) > 1
    ), bw_paths
    # get pearson r
...
def get_replicates(
    ...
    repl_labels=["r2", "r12"],
    basenji_samplefiles=[
        "/mnt/906427d6-fddf-41bf-9ec6-c3d0c37e766f/amber/ATAC/basenji_sample_r2_file.tsv",
        "/mnt/906427d6-fddf-41bf-9ec6-c3d0c37e766f/amber/ATAC/basenji_sample_r1,2_file.tsv",
    ],
):
    ...
    for b, basenji_samplefile in enumerate(basenji_samplefiles):
        ...
        cell_row = basenji_samplefile_df[
            basenji_samplefile_df["identifier"] == cell_line_name
        ]["file"]
...
def get_idr(
    ...
    window_enf_idr = "/".join(
        split_filename[:-1] + [split_filename[-1].split(".bed")[0] + "_const" + ".bed"]
    )
    ...
    idr_file_gz = basset_samplefile_df[basset_samplefile_df[0] == cell_line_name][
        1
    ].values[0]
    ...
    make_bedfile = "scp {} temp.bed.gz; gunzip temp.bed.gz; grep chr8 temp.bed|awk '{{print $1, $2, $3}}'|sort -k1,1 -k2,2n|uniq > {}; rm temp.bed".format(
        idr_file_gz, interm_bed
    )
    ...
    filter_bed = "bedtools intersect -v -a {} -b {} > {}".format(
        nan_window_enf_idr, unmap_bed, window_enf_idr
    )
    process = subprocess.Popen(filter_bed, shell=True)
    output, error = process.communicate()
    # merge ranges so that bw writing can happen later
    merge_bed = "bedtools merge -i {} > {}; rm {}; rm {}".format(
        window_enf_idr, idr_filename, interm_bed, nan_window_enf_idr
    )
...
def bw_from_ranges(
    ...
    for line in in_bedfile:  # per bed range
        ...
        if np.max(vals) > threshold:  # if above threshold
            ...
            out_bw.addEntries(
                cols[0], int(cols[1]), values=vals, span=bin_size, step=bin_size
            )
...
def save_jointplot(cov_dict, save_path, cell_line_name):
    ...
    joint_grid = sns.jointplot(
        ...
        color=c,
        kind="reg",
        joint_kws={"scatter_kws": dict(alpha=a)},
    )
...
def scatter_cell_line_prs(corr_dfs, savedir):
    ...
    avg_df = melted_corrs["truth"].merge(
        melted_corrs["pred"],
...
def plot_corr_matrices(corr_dfs, savedir):
    ...
    truth_heatmap = sns.heatmap(
        corr_dfs["truth"], annot=True, vmin=min_lim, vmax=1, ax=axs[0], cmap="flare"
    )
    axs[0].set_title(titles[0])
    pred_heatmap = sns.heatmap(
        corr_dfs["pred"], annot=True, vmin=min_lim, vmax=1, ax=axs[1], cmap="flare"
    )
...
def process_run(
    ...
    for subdir in tqdm(os.listdir(run_subdir)):  # per cell line directory
        ...
        # define IDR bw path
        cell_line_truth_idr = os.path.join(
            output_dir, cell_line_name + "_truth_1_idr.bed"
        )
        ...
        if get_replicates:  # if replicates needed
            ...
            for rX, rX_bw_path in replicate_filepaths.items():  # per replicate
                ...
                out_rX = os.path.join(
                    output_dir, cell_line_name + "_{}_1_raw.bw".format(rX)
                )
        ...
        for in_bw in rX_bw_filenames + [truth_bw_filename]:
            ...
            if bin_size != 1:  # if bin less than one don't redo bw making!
                bw_from_ranges(
                    in_bw, bed_filename, out_bw, chrom_size_path, bin_size=bin_size
                )
        ...
        for binned_filename in binned_filenames + [pred_bw_filename]:  # for all new bws
            ...
            bw_from_ranges(
                binned_filename, cell_line_truth_idr, out_bw, chrom_size_path
            )
        ...
        if threshold > 0:
            ...
            thresh_bedfile = truth_bw_filename.split(".bw")[0] + "_{}.bed".format(
                thresh_str
            )
            # new bw filename for ground truth
            truth_thresh_filename = change_filename(
                truth_bw_filename, new_thresholdmethod=thresh_str
            )
            bw_from_ranges(
                truth_bw_filename,
                bed_filename,
                truth_thresh_filename,
                chrom_size_path,
                threshold=threshold,
                out_bed_filename=thresh_bedfile,
            )
            ...
            for binned_filename in binned_filenames + [pred_bw_filename]:
                ...
                out_thresh = change_filename(
                    binned_filename, new_thresholdmethod=thresh_str
                )
                if (
                    "truth_1_thresh" not in out_thresh
                ):  # this one would already be made above
                    bw_from_ranges(
                        binned_filename, thresh_bedfile, out_thresh, chrom_size_path
                    )
...
def evaluate_run_performance(run_dir, rm_bws=False, rm_all=False):
    ...
    folders_of_cell_lines = [
        os.path.join(bigwigs_dir, f)
        for f in os.listdir(bigwigs_dir)
        if os.path.isdir(os.path.join(bigwigs_dir, f))
    ]
    ...
    cell_line_names = [
        cell_line_dir.split("/")[-1].split("_")[1]
        for cell_line_dir in folders_of_cell_lines
    ]
    ...
    for c, cell_line_dir in tqdm(enumerate(folders_of_cell_lines)):
        ...
        plot_paths = {
            bw_t: os.path.join(
                summary_dir, "{}_{}_scatter.svg".format(bw_t, cell_line_name)
            )
            for bw_t in ["raw", "idr"]
        }
        print(cell_line_dir)
        ...
        t_bw, p_bw = [
            os.path.join(
                cell_line_dir, "{}_{}_{}_raw.bw".format(cell_line_name, x, bin_size)
            )
            for x in ["truth", "pred"]
        ]
        raw_bed = os.path.join(
            cell_line_dir, "{}_truth_1_raw.bed".format(cell_line_name)
        )
        idr_bed = os.path.join(
            cell_line_dir, "{}_truth_1_idr_const.bed".format(cell_line_name)
        )
        assert all(
            [os.path.isfile(f) for f in [t_bw, p_bw, raw_bed, idr_bed]]
        ), "One or more of the files not found in {}".format(cell_line_dir)
        np_cov_dict = {}
        for bw_type, bed_filename in [("raw", raw_bed), ("idr", idr_bed)]:
            np_cov_dict[bw_type] = {}
            for fold_type, bw_filename in [("truth", t_bw), ("pred", p_bw)]:
                np_cov_dict[bw_type][fold_type] = get_vals_per_range(
                    bw_filename, bed_filename
                )
        for bw_type in ["raw", "idr"]:
            ...
            summary_line += get_metrics(
                np_cov_dict[bw_type]["truth"], np_cov_dict[bw_type]["pred"], bw_type
            )
            save_jointplot(
                np_cov_dict[bw_type], plot_paths[bw_type], cell_line_name
            )  # plot raw
            avg_cov_dict = {}  # save avg for correlation matrix
            avg_cov_dict[bw_type] = {
                bw_source: avg_cov(np_cov_dict[bw_type][bw_source])
                for bw_source in ["truth", "pred"]
            }
            if bw_type == "raw":
