...
_, _, input_size, _, window, model_name, loss_name = grid_point.split(".h5")[0].split(
    "_"
)
...
def load_basenji(model_path):
    grid_point = os.path.basename(model_path)
    _, _, input_size, _, window, model_name, loss_name = grid_point.split(".h5")[
        0
    ].split("_")
    ...
    model = tf.keras.models.load_model(
        model_path, custom_objects=custom_layers, compile=False
    )
...
y_test = util.tfr_to_np(
    test_data, "y", (params["test_seqs"], params["seq_length"], params["num_targets"])
)
...
for i in range(0, num_task):
    ...
    saliency_map = explain.peak_saliency_map(
        X, model, class_index=i, window_size=window
    )
...
plt.savefig(
    "/home/shush/profile/QuantPred/datasets/top25/grid2/saliency_" + fig_name + ".pdf"
)
