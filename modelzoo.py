...
def basenjimod(input_shape, output_shape, wandb_config={}):
    ...
    layer_dict = {
        32: [1, False],  # if bin size 32 add 1 maxpool and no maxpool of size 2
    ...
    current, rep_filters = conv_tower(
        current,
        filters_init=config["filtN_2"],
        filters_mult=config["filt_mlt"],
        repeat=n_conv_tower,
        pool_size=4,
        kernel_size=5,
        dropout=drp2,
    ...
    if add_2max:
        ...
        current = conv_block(
            ...
            kernel_size=5,
            activation="gelu",
            activation_end=None,  # changed filter size 5
            strides=1,
            dilation_rate=1,
            l2_scale=0,
            dropout=drp3,
    ...
    outputs = dense_layer(
        current, n_exp, activation="softplus", batch_norm=True, bn_momentum=0.9
    )
...
def basenjiw1(input_shape, output_shape, wandb_config={}):
    """
    Basenji model turned into a single function.
    inputs (None, seq_length, 4)
    """
    ...
    filtN_list = [
        config[f] for f in ["filtN_1", "filtN_2", "filtN_3", "filtN_4", "filtN_5"]
    ]
    filt_drp_dict = {64: 0.1, 128: 0.2, 256: 0.3, 512: 0.4, 1024: 0.5}
    if config["add_dropout"]:
    ...
    current = dilated_residual(
        current,
        filters=config["filtN_3"],
        kernel_size=3,
        rate_mult=config["mult_rate1"],
        conv_type="standard",
        dropout=0.25,
        repeat=2,
        round=False,  # repeat=4
    ...
    for n in range(2):
        ...
        current = tf.keras.layers.Conv2DTranspose(
            filters=n_filters, kernel_size=(5, 1), strides=(2, 1), padding="same"
        )(current)
        current = tf.keras.layers.UpSampling2D(size=(2, 1))(current)

    current = tf.keras.layers.Conv2DTranspose(
        filters=mult_filt(n_filters), kernel_size=(5, 1), strides=(2, 1), padding="same"
    )(current)
    current = tf.keras.layers.Conv2D(mult_filt(n_filters), 1)(current)
    current = dense_layer(
        current, n_exp, activation="softplus", batch_norm=True, bn_momentum=0.9
    )
    outputs = tf.squeeze(current, axis=2)
    # upsample
...
def bpnet(input_shape, output_shape, wandb_config={}):
    ...
    for task in range(0, output_shape[1]):
        ...
        px = keras.layers.Conv2DTranspose(
            config["strand_num"], kernel_size=(config["kern_3"], 1), padding="same"
        )(px)
        px = keras.layers.Reshape((-1, config["strand_num"]))(px)
        px = keras.layers.AveragePooling1D(
            pool_size=window_size, strides=None, padding="valid"
        )(px)
        outputs.append(px)
...
def lstm(input_shape, output_shape):
    ...
    conv1_residual = dilated_residual_block(
        conv1, num_filters=64, filter_size=7, activation="relu", l2=1e-6
    )
    conv1_residual_pool = keras.layers.MaxPool1D(
        pool_size=10, strides=5, padding="same"
    )(conv1_residual)
    ...
    conv2_residual = dilated_residual_block2(
        conv2, num_filters=64, filter_size=11, activation="relu", l2=1e-6
    )
    conv2_residual_pool = keras.layers.MaxPool1D(
        pool_size=25, strides=5, padding="same"  # TODO: make larger - 25
    )(conv2_residual)
    ...
    nn = tf.keras.layers.Dense(output_shape[0] * output_shape[1], activation="relu")(
        avg_pool
    )
    gelu_layer = GELU()
    nn = gelu_layer(nn)
...
def dilated_residual_block(
    input_layer, num_filters, filter_size, activation="relu", l2=None
):
    if not l2:
        l2 = keras.regularizers.l2(l2)
    else:
    ...
    residual_conv1 = keras.layers.Conv1D(
        filters=num_filters,
    ...
    residual_conv2 = keras.layers.Conv1D(
        filters=num_filters,
        kernel_size=filter_size,
        strides=1,
        activation=None,
    ...
    residual_conv3 = keras.layers.Conv1D(
        filters=num_filters,
        kernel_size=filter_size,
        strides=1,
        activation=None,
...
def load_model(run_dir, compile):
    ...
    model = tf.keras.models.load_model(
        best_model, custom_objects=custom_layers, compile=False
    )
