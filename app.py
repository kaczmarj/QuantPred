...
def get_data(pred, cell_line=1):
    ...
    pr_all = [
        scipy_pr(test_y[seq_n, :, cell_line], test_pred[seq_n, :, cell_line])
        for seq_n in range(test_y.shape[0])
    ]
    df = pd.DataFrame(
        {
            "true_cov": test_y.mean(axis=1)[:, cell_line],
            "pred_cov": test_pred.mean(axis=1)[:, cell_line],
        }
    )
    data = {
        "raw": (test_y[:, :, cell_line], test_pred[:, :, cell_line]),
        "coords": coords,
        "avg": df,
        "pr_all": pr_all,
        "peak": colors,
...
def create_scatter(pred=DATA_DIR, cell_line=CELL_LINE):
    ...
    fig = px.scatter(
        ...
        x="true_cov",
        y="pred_cov",
        title="Average true vs pred",
        opacity=0.6,
        color="peak",
        color_continuous_scale="viridis",
    )
    fig.add_scatter(
        x=np.arange(0, 4), y=np.arange(0, 4), mode="lines", hoverinfo="skip", name=""
    )
...
app.layout = html.Div(
    [
        ...
        html.Div(
            [dcc.Graph(id="predictions", figure=fig)],
            style={"width": "49%", "display": "inline-block"},
        ),
        html.Div(
            [dcc.Graph(id="profile")], style={"width": "49%", "display": "inline-block"}
        )
        #
        # ])
    ]
...
def update_profile(hoverData, pred=DATA_DIR, cell_line=CELL_LINE):
    ...
    fig = px.line(
        x=np.arange(len(data["raw"][0][seq_n, :])),
        y=data["raw"][0][seq_n, :],
        title="Coverage, Pearson R={}, Spearman corr={}, {}".format(
            np.around(pr, 3),
            np.around(sc, 3),
    ...
    fig.add_scatter(
        x=np.arange(len(data["raw"][1][seq_n, :])),
        y=data["raw"][1][seq_n, :],
        name="Predicted",
        mode="lines",
