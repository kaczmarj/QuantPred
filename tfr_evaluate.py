...
def evaluate_idr(run_path, log_all):
    cl_datasets = glob.glob(
        "/home/shush/profile/QuantPred/datasets/cell_line_specific_test_sets/cell_line_*/complete/peak_centered/i_2048_w_1/"
    )
    one_run_all = []
    for data_dir in cl_datasets:
        sts = util.load_stats(data_dir)
        testset = util.make_dataset(
            data_dir, "test", sts, batch_size=512, shuffle=False
        )
        targets = pd.read_csv(data_dir + "targets.txt", sep="\t")["identifier"].values
        i = int(data_dir.split("cell_line_")[-1].split("/complete")[0])
        performance = evaluate_per_cell_line(
            run_path, testset, targets, log_all, choose_cell=i
        )
...
def get_true_pred(run_path, testset, log_all=False):
    ...
    if log_all:
        ...
        return np.log(np.concatenate(all_truth) + 1), np.log(
            np.concatenate(all_pred) + 1
        )
    else:
...
if __name__ == "__main__":
    data_dir = (
        "/home/shush/profile/QuantPred/datasets/chr8/complete/random_chop/i_2048_w_1/"
    )
    sts = util.load_stats(data_dir)
