...
class RobustTrainer:
    ...
    def robust_train_epoch(
        ...
        for i, (x, y) in enumerate(batch_dataset):
            ...
            progress_bar(
                i + 1, num_step, start_time, bar_length=30, loss=running_loss / (i + 1)
            )
    ...
    def robust_evaluate(
        self, name, dataset, batch_size=128, verbose=True, training=False
    ):
    ...
    def check_early_stopping(self, name="val"):
        ...
        return self.early_stopping.status(
            self.metrics[name].get(self.early_stopping.metric)[-1]
        )

    def set_lr_decay(self, decay_rate, patience, metric="loss", criterion=None):
        """set up learning rate decay"""
        self.lr_decay = LRDecay(
...
class LRDecay:
    def __init__(
        self, optimizer, decay_rate=0.3, patience=10, metric="loss", criterion=None
    ):
...
class MonitorMetrics:
    ...
    def update(self):
        ...
        self.running_loss = []
        for metric_name in self.metric_names:
            self.metric[metric_name].append(
                np.mean(self.metric_update[metric_name].result())
            )
        self.reset()
    ...
    def print(self):
        ...
        for metric_name in self.metric_names:
            print(
                ...
                + " "
                + metric_name
                + ":\t{:.5f}".format(self.metric[metric_name][-1])
            )
...
def random_crop(x, y, window_size):
    ...
    if x_dim[1] > window_size:
        indice = (
            ...
            + np.random.randint(low=0, high=x_dim[1] - window_size, size=x_dim[0])[
                :, np.newaxis
            ]
        )
...
def bin_resolution(y, bin_size):
    ...
    y_bin = tf.math.reduce_mean(
        tf.reshape(y, (y_dim[0], int(y_dim[1] / bin_size), bin_size, y_dim[2])), axis=2
    )
    return y_bin
...
def progress_bar(iter, num_batches, start_time, bar_length=30, **kwargs):
    ...
    if iter == num_batches:  # if last batch, then output total elapsed time
        ...
        remaining_time = (
            (time.time() - start_time) * (num_batches - (iter + 1)) / (iter + 1)
        )
