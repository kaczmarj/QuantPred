...
def smooth_one_binsize_one_cell_line(file, bin_size=2048, out_dir="gauss_smooth"):
    ...
    in_bed_filename = (
        "/home/shush/profile/QuantPred/bin_exp/truth/merged_truth_1_raw.bed"
    )
    subdirs = os.path.join(true_binned_dir, file)
    print("Porcessing cell line {}".format(file))
    true_raw_bw_list = [
        os.path.join(subdirs, bw_file)
        for bw_file in os.listdir(subdirs)
        if "_{}_raw.bw".format(bin_size) in bw_file
    ]
    assert len(true_raw_bw_list) == 1, "Too many or not enough bws found in {}!".format(
        subdirs
    )
    true_raw_bw = true_raw_bw_list[0]
    pred_raw_bw = true_raw_bw.replace("truth", "pred").replace(
        "_{}_".format(bin_size), "_{}_{}_".format(bin_size, bin_size)
    )
    ...
    df = pd.DataFrame(
        summary_results, columns=["bin_size", "sigma", "pearson_r", "cell_line"]
    )
