...
def task_robustness(
    ...
    while i < len(selected_read):
        ...
        crop_f_index = np.vstack((crop_row_idx, crop_idx)).T.reshape(
            shift_num * batch_n, conserve_size, 2
        )
        ...
        if visualize == True:
            ...
            for a in range(0, batch_n):
                ...
                for shift_n in range(0, shift_num):
                    ...
                    sns.lineplot(
                        x=center_range + shift_idx[shift_n],
                        y=shift_pred[a * shift_num + shift_n, :],
                        ax=ax1,
                        alpha=0.35,
                    )
                    # visualize saliency
                    ...
                    sns.lineplot(
                        x=center_range + shift_idx[shift_n],
                        y=np.sum(tmp_saliency.numpy(), axis=1),
                        ax=ax2,
                        alpha=0.35,
                    )
                ...
                if smooth_saliency == True:
                    ...
                    sns.lineplot(
                        x=range(conserve_start, conserve_end + 1),
                ...
                sns.lineplot(x=range(0, conserve_size), y=line_saliency, ax=ax3)
                ax3.fill_between(
                    range(0, conserve_size),
...
def batch_robustness_test(
    ...
    while i < len(selected_read):
        ...
        # get pred 1k part
        shift_pred_1k = tf.gather_nd(
            shift_pred[range(shift_pred.shape[0]), :, max_task], crop_f_index
        )
        sep_pred = np.array(np.array_split(shift_pred_1k, batch_n))
        var_pred = np.std(sep_pred, axis=1)
        var_pred_sum = np.sum(var_pred, axis=1)
        ...
        if visualize == True:
            ...
            for a in range(0, batch_n):
                ...
                if ground_truth == True:
                    ...
                    sns.lineplot(
                        x=range(0, chop_size),
                        y=np.squeeze(target[a, :, short_max_task[a]]),
                        ax=ax1,
                        color="lightblue",
                    )
                ...
                for shift_n in range(0, shift_num):
                    ...
                    sns.lineplot(
                        x=center_range + shift_idx[shift_n],
                        y=shift_pred[a * shift_num + shift_n, :, short_max_task[a]],
                        ax=ax1,
                        alpha=0.35,
                    )
                    # visualize saliency
...
def plot_saliency(saliency_map):
    ...
    fig, axs = plt.subplots(
        saliency_map.shape[0], 1, figsize=(200, 5 * saliency_map.shape[0])
    )
...
def vcf_test(ref, alt, coords, model, background_size=100):
    ...
    for i, ref_seq in enumerate(ref):
        ...
        mut_base = np.apply_along_axis(
            np.random.choice, axis=1, arr=ori_empty_base, size=1
        )
...
def visualize_vcf(ref, alt, model, background_size=100):
    ...
    plt.fill_between(
        range(0, mut_pred.shape[1]),
        np.squeeze(mut_pred.max(axis=0)),
        np.squeeze(mut_pred.min(axis=0)),
        facecolor="grey",
    )
...
def peak_saliency_map(X, model, class_index, window_size, func=tf.math.reduce_mean):
    ...
    with tf.GradientTape() as tape:
        ...
        for i in range(0, X.shape[0]):
            column_indices = tf.range(
                peak_index[i] - int(bin_num / 2),
                peak_index[i] + math.ceil(bin_num / 2),
                dtype="int32",
            )
            row_indices = tf.keras.backend.repeat_elements(
                tf.constant([i]), bin_num, axis=0
            )
...
def combine_beds(samplefile, out_path):
    ...
    combined_csv = pd.concat(
        [
            (pd.read_csv(f, sep="\t", header=None).iloc[:, :3]).drop_duplicates()
            for f in bed_paths
        ]
    )
...
def filter_dataset(dsq_path, out_path="dsq_all.bed"):
    ...
    dsq_filt[["a1", "a2"]] = dsq_filt["genotypes"].str.split(
        "/", expand=True
    )  # write into separate columns
    dsq_filt.to_csv(out_path, sep="\t", header=False, index=None)
    return dsq_filt


def bed_intersect(dataset_bed, comb_peak, out_path):
    bashCmd = "bedtools intersect -wa -a {} -b {} > {}".format(
        dataset_bed, comb_peak, out_path
    )
...
def extend_ranges(column_names, bedfile, out_path, window):
    ...
    dsq_nonneg["counts"] = (
        dsq_nonneg.groupby(["chrom", "snpChromStart"])["snpChromStart"]
        .transform("count")
        .values
    )
...
def bed_to_fa(
    ...
    bashCmd = "bedtools getfasta -fi {} -bed {} -s -fo {}".format(
        genome_file, bedfile, out_fa
    )
    process = subprocess.Popen(bashCmd, shell=True)
    output, error = process.communicate()
    print(error)
...
def onehot_to_h5(
    onehot_ref, onehot_alt, coord_np, pct_sign, out_dir=".", filename="onehot.h5"
):
...
def table_to_h5(
    ...
    out_peaks="combined_atac.bed",
    out_filt="dsq_all.bed",
    out_open="dsq_open.bed",
    out_fin="filt_open_ext.bed",
    out_fa="ext.fa",
    genome_file="/home/shush/genomes/hg19.fa",
    window=3072,
    out_dir=".",
    ...
    dsq_nonneg, counts_per_cell, pct_sign = extend_ranges(
        column_names, out_open, out_fin, window
    )
    ...
    onehot_ref, onehot_alt, coord_np = str_to_onehot(
        coords_list, seqs_list, dsq_nonneg, window
    )
    print("Saving onehots as h5")
    onehot_to_h5(
        onehot_ref, onehot_alt, coord_np, counts_per_cell, pct_sign, out_dir, out_h5
    )
...
def merge_one_cell_line(chr8_dsq_file, idr_bed, i="0", out_merged_bed="merged.bed"):
    ...
    bashCmd = "bedtools intersect -wa -a {} -b {} > {}".format(
        chr8_dsq_file, idr_bed, out_merged_bed
    )
    process = subprocess.Popen(bashCmd, shell=True)
    output, error = process.communicate()
...
def get_h5_with_cells(
    ...
    merged_vcfs = (
        conc_vcfs.groupby(["chrom", "snpChromStart", "snpChromEnd"])["idr_N"]
        .apply(", ".join)
        .reset_index()
    )
    # reattach columsn with metadata
    complete_df = merged_vcfs.merge(
        conc_vcfs, how="left", on=["chrom", "snpChromStart", "snpChromEnd"]
    )
    # remove redundant columns and rows
    complete_unq = (
        complete_df.drop(columns="idr_N_y").drop_duplicates().reset_index(drop=True)
    )
    # get the 3K range start and end
    ...
    complete_unq = complete_unq[
        complete_unq["3K_start"] > 0
    ]  # remove ones starting at negative coords
    complete_unq.rename(
        columns={"idr_N_x": "cell_lines"}, inplace=True
    )  # rename column
    complete_unq.to_csv(
        os.path.join(out_dir, "vcf_metadata.csv"), sep="\t", index=None
    )  # save the complete metadata table
    # save version needed for fa conversion
    complete_unq[
        ["chrom", "3K_start", "3K_end", "rsid", "pred.fit.pctSig", "strand"]
    ].to_csv("out_fin.bed", sep="\t", header=None, index=None)
    bed_to_fa("out_fin.bed", "out.fa", genome_file="/home/shush/genomes/hg19.fa")
    coords_list, seqs_list = fasta2list("out.fa")
    onehot_ref, onehot_alt, coord_np = str_to_onehot(
        coords_list, seqs_list, complete_unq, window
    )
    ...
    onehot_to_h5(
        ...
        onehot_alt,
        coord_np,
        np.array(complete_unq["pred.fit.pctSig"].values),
        out_dir,
    )
