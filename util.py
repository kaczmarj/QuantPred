...
def make_dataset(
    ...
    dataset = dataset.map(
        generate_parser(seq_length, target_length, num_targets, coords)
    )
    if shuffle:
        if seed:
...
def make_dataset_binned(
    ...
    dataset = dataset.map(
        generate_parser_with_binning(
            seq_length, target_length, num_targets, coords, bin_size=128
        )
    )
    if shuffle:
        if seed:
...
def window_shift(X, Y, window_size, shift_num):
    ...
    shift_idx = (
        ...
        + np.random.randint(low=0, high=chop_size - window_size, size=output_num)[
            :, np.newaxis
        ]
    )
